...
import { Shield, Brain, Zap, User, Camera, Plus, Dices, Swords, Save, ArrowLeft, Trash2, Minus, Sparkles, Flame, Ghost } from 'lucide-react';

const cinzel = Cinzel({ subsets: ['latin'], weight: ['400', '700', '900'] });
const inter = Inter({ subsets: ['latin'], weight: ['400', '600'] });

// --- DICIONÁRIO DE SISTEMAS (PRESETS) ---
const PRESETS = {
  ordem_paranormal: {
    origens: ["Acadêmico", "Agente de Saúde", "Amnésico", "Atleta", "Criminoso", "Cultista Arrependido", "Investigador", "Lutador", "Militar", "Religioso", "TI"],
  ...
  dnd5e: {
    origens: ["Acólito", "Charlatão", "Criminoso", "Entretenimento", "Herói do Povo", "Nobre", "Sábio", "Soldado"],
    classes: ["Bárbaro", "Bardo", "Clérigo", "Druida", "Guerreiro", "Ladino", "Mago", "Paladino", "Feiticeiro"],
    categorias_hab: [
      { id: 'comum', nome: 'Características', icon: Swords },
      { id: 'magias', nome: 'Magias', icon: Sparkles }
    ]
  },
  memorias_postumas: {
    origens: ["Sobrevivente", "Cientista", "Militar", "Ocultista"],
    classes: ["Vanguarda", "Suporte", "Assalto"],
    categorias_hab: [
      { id: 'comum', nome: 'Habilidades', icon: Swords },
...
export default function FichaPersonagemPage() {
  ...
  const recalcularMaximos = (dadosAtuais: any, sistema: string) => {
    ...
    // D&D e Memórias (Baseados em Level)
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="flex flex-col md:flex-row justify-between gap-6 mb-12 border-b border-[#2a3b52] pb-8">
          <div className="flex-1 space-y-2">
            ...
            <div className="flex gap-2">
               <select value={ficha.sistema_preset} onChange={(e) => { mudarSistema(e.target.value); }} className="bg-[#131b26] border border-[#2a3b52] text-[10px] text-[#4ad9d9] px-2 py-1 rounded uppercase font-bold outline-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div onClick={() => fileInputRef.current?.click()} className="relative aspect-[3/4] bg-[#131b26] border border-[#2a3b52] rounded-3xl overflow-hidden group cursor-pointer">
              <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" />
              {isUploading ? <div className="absolute inset-0 flex items-center justify-center bg-black/60 z-10 text-[#4ad9d9] animate-pulse font-bold">ENVIANDO...</div> : null}
              {ficha.dados.avatar_url ? <img src={ficha.dados.avatar_url} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" /> : <div className="w-full h-full flex flex-col items-center justify-center text-[#2a3b52] group-hover:text-[#4ad9d9]"><Camera size={48}/><span className="text-[10px] mt-4 font-bold tracking-widest">DEFINIR RETRATO</span></div>}
            </div>

            <div className="bg-[#131b26] border border-[#2a3b52] rounded-3xl p-6 space-y-6 relative overflow-hidden">
               ...
               <div className="space-y-4 pt-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={ficha.dados.origem} onChange={(e) => atualizarFicha('origem', e.target.value)} className="w-full bg-transparent border-b border-[#2a3b52] text-white py-1 outline-none text-sm">
                    ...
                    <div>
                      ...
                      <select value={ficha.dados.classe} onChange={(e) => atualizarFicha('classe', e.target.value)} className="w-full bg-transparent border-b border-[#2a3b52] text-white py-1 outline-none text-sm">
                        <option value="">Selecionar...</option>
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {['vida', 'sanidade', 'estamina'].map(s => {
                ...
                return (
                  <div key={s} className="bg-[#131b26] border border-[#2a3b52] rounded-3xl p-5 group">
                    <div className="flex justify-between items-end mb-3">
                      <span className="text-[10px] uppercase font-black tracking-widest" style={{ color }}>{label}</span>
                      <div className="flex items-center gap-1">
                        <input type="number" value={stat.atual} onChange={(e) => atualizarStatusAtual(s, e.target.value)} className="bg-transparent text-right font-black text-xl text-white w-12 outline-none" />
                    ...
                    <div className="flex justify-between mt-4">
                      <button onClick={() => atualizarStatusAtual(s, '-1', true)} className="p-2 hover:bg-white/5 rounded-lg transition-colors"><Minus size={14}/></button>
                      <button onClick={() => atualizarStatusAtual(s, '1', true)} className="p-2 hover:bg-white/5 rounded-lg transition-colors"><Plus size={14}/></button>
                    </div>
                  </div>
                );
              })}
            ...
            {/* ATRIBUTOS */}
            <section className="bg-[#131b26] border border-[#2a3b52] rounded-3xl p-8">
            ...
            {/* HABILIDADES COM ABAS CATEGORIZADAS */}
            <section className="bg-[#131b26] border border-[#2a3b52] rounded-3xl overflow-hidden">
               <div className="flex border-b border-[#2a3b52]">
                 {currentSys?.categorias_hab.map(cat => (
                   <button 
                     key={cat.id} 
                     onClick={() => setActiveTab(cat.id)}
                     className={`flex-1 py-4 px-6 text-[10px] uppercase font-black tracking-widest flex items-center justify-center gap-2 transition-all ${activeTab === cat.id ? 'bg-[#1a2b4c] text-[#4ad9d9]' : 'text-[#6b7b94] hover:bg-white/5'}`}
                   >
                     <cat.icon size={14}/> {cat.nome}
                   </button>
                 ))}
               </div>
               ...
               <div className="p-8">
                 ...
                 {isAddingSkill && (
                   <div className="flex flex-col md:flex-row gap-3 mb-8 p-4 bg-[#0a0f18] border border-[#4ad9d9]/30 rounded-2xl">
                     <input type="text" placeholder="Nome do Poder/Magia" value={newSkill.nome} onChange={(e) => setNewSkill({...newSkill, nome: e.target.value})} className="bg-transparent border-b border-[#2a3b52] text-sm text-white px-2 py-2 w-full outline-none focus:border-[#4ad9d9]" />
                     <input type="text" placeholder="Dado/Custo" value={newSkill.dado} onChange={(e) => setNewSkill({...newSkill, dado: e.target.value})} className="bg-transparent border-b border-[#2a3b52] text-sm text-white px-2 py-2 w-full md:w-32 outline-none focus:border-[#4ad9d9]" />
                     <button onClick={adicionarHabilidade} className="bg-[#4ad9d9] text-[#090e17] px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest">Adicionar</button>
                   </div>
                 )}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                   ...
                   {ficha.dados.habilidades.filter((h: any) => h.cat === activeTab).map((h: any) => (
                     <div key={h.id} className="group flex justify-between items-center bg-[#0a0f18] border border-[#1a2b4c] p-4 rounded-2xl hover:border-[#4ad9d9]/50 transition-all">
