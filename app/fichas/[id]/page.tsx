...
export default function FichaPersonagemPage() {
  ...
  const carregarFicha = async () => {
    ...
    if (data) {
      ...
      // Compatibilidade: app/page.tsx salva como "attributes" (inglês), esta página usa "atributos"
      if (!data.dados.atributos && data.dados.attributes) {
        data.dados.atributos = data.dados.attributes;
      }
      // Garante que todos os campos existem e são números válidos
      const atrBase = data.dados.atributos || {};
      data.dados.atributos = {
        forca:     Number(atrBase.forca)     || 1,
        agilidade: Number(atrBase.agilidade) || 1,
        vigor:     Number(atrBase.vigor)     || 1,
        intelecto: Number(atrBase.intelecto) || 1,
        presenca:  Number(atrBase.presenca)  || 1,
      };
  ...
  const recalcularMaximos = (dadosAtuais: any, sistema: string) => {
    // Coerção defensiva: lê de 'atributos' ou 'attributes' (compatibilidade app/page.tsx)
    // e garante número válido mesmo se chave estiver faltando ou null
    const raw = dadosAtuais.atributos || dadosAtuais.attributes || {};
    const atr = {
      forca:     Number(raw.forca)     || 1,
      agilidade: Number(raw.agilidade) || 1,
      vigor:     Number(raw.vigor)     || 1,
      intelecto: Number(raw.intelecto) || 1,
      presenca:  Number(raw.presenca)  || 1,
    };
    const nex = Number(dadosAtuais.nex) || 5;
