-- AetherQuest: motor de mesa online, membros de sala e permissoes do mestre.
-- Rode este arquivo no SQL Editor do Supabase antes de testar o VTT em producao.
...
on public.sala_membros for select to authenticated
using (
  user_id = (select auth.uid())
  or exists (select 1 from public.salas s where s.id = sala_membros.sala_id and s.user_id = (select auth.uid()))
);

create policy sala_membros_insert_self_safe_role
on public.sala_membros for insert to authenticated
with check (
  user_id = (select auth.uid())
  and (
    role = 'jogador'
...
on public.sala_membros for update to authenticated
using (exists (select 1 from public.salas s where s.id = sala_membros.sala_id and s.user_id = (select auth.uid())))
with check (exists (select 1 from public.salas s where s.id = sala_membros.sala_id and s.user_id = (select auth.uid())));

create policy sala_membros_delete_self_or_room_master
on public.sala_membros for delete to authenticated
using (
  user_id = (select auth.uid())
  or exists (select 1 from public.salas s where s.id = sala_membros.sala_id and s.user_id = (select auth.uid()))
);
...
on public.cenas for insert to authenticated
with check (exists (select 1 from public.salas s where s.id::text = cenas.sala_id::text and s.user_id = (select auth.uid())));

create policy cenas_update_room_master
on public.cenas for update to authenticated
using (exists (select 1 from public.salas s where s.id::text = cenas.sala_id::text and s.user_id = (select auth.uid())))
with check (exists (select 1 from public.salas s where s.id::text = cenas.sala_id::text and s.user_id = (select auth.uid())));

create policy cenas_delete_room_master
on public.cenas for delete to authenticated
using (exists (select 1 from public.salas s where s.id::text = cenas.sala_id::text and s.user_id = (select auth.uid())));
...
with check (exists (select 1 from public.salas s where s.id::text = tokens.sala::text and s.user_id = (select auth.uid())));

create policy tokens_update_room_master_or_ficha_owner
on public.tokens for update to authenticated
using (
  exists (select 1 from public.salas s where s.id::text = tokens.sala::text and s.user_id = (select auth.uid()))
  or exists (select 1 from public.fichas f where f.id::text = tokens.ficha_id::text and f.user_id = (select auth.uid()))
)
with check (
  exists (select 1 from public.salas s where s.id::text = tokens.sala::text and s.user_id = (select auth.uid()))
  or exists (select 1 from public.fichas f where f.id::text = tokens.ficha_id::text and f.user_id = (select auth.uid()))
...
using (exists (select 1 from public.salas s where s.id::text = tokens.sala::text and s.user_id = (select auth.uid())));

create policy fichas_select_owner_or_room_master
on public.fichas for select to authenticated
using (
  user_id = (select auth.uid())
  or exists (select 1 from public.salas s where s.id::text = fichas.sala_id::text and s.user_id = (select auth.uid()))
  or exists (
    select 1 from public.tokens t
    join public.salas s on s.id::text = t.sala::text
    where t.ficha_id::text = fichas.id::text and s.user_id = (select auth.uid())
  )
);
