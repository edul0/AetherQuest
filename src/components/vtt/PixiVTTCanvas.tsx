...
type DragState =
  ...
  | { type: "measure"; startWorld: Point; endWorld: Point }
  | { type: "fog"; painting: boolean }
  | null;

// Fog cell key = "gx,gy" in grid units
type FogMap = Set<string>;
...
const METERS_PER_CELL = 1.5; // each grid cell = 1.5m by default
const CONDITION_HEX: Record<string, number> = {
  dead: 0xef4444, morto: 0xef4444,
  unconscious: 0x6b7280, inconsciente: 0x6b7280,
  poisoned: 0x22c55e, envenenado: 0x22c55e,
  burning: 0xf97316, queimando: 0xf97316,
  stunned: 0xf59e0b, atordoado: 0xf59e0b,
  prone: 0xa78bfa, caido: 0xa78bfa,
  blinded: 0x374151, cego: 0x374151,
  concentration: 0x4ad9d9,
};
...
export default function PixiVTTCanvas({
  ...
  const [fogMap, setFogMap] = useState<FogMap>(new Set<string>());
  const [measureLine, setMeasureLine] = useState<{ start: Point; end: Point } | null>(null);
  const fogMapRef = useRef<FogMap>(new Set<string>());
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      world.addChild(grid, tokenLayer, fogLayer, measureLayer);
  ...
  const worldToCell = useCallback(
    (worldPoint: Point): Point => {
      const gs = scenePreferences.gridSize || 50;
      return { x: Math.floor(worldPoint.x / gs), y: Math.floor(worldPoint.y / gs) };
    },
    [scenePreferences.gridSize],
  );

  const screenToCell = useCallback(
    (screenPoint: Point): Point => worldToCell(screenToWorld(screenPoint)),
    [worldToCell, screenToWorld],
  );

  const toggleFogCell = useCallback((cellKey: string, painting: boolean) => {
    setFogMap((current) => {
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (scenePreferences.toolMode === "measure") {
      ...
      dragRef.current = { type: "measure", startWorld: worldPoint, endWorld: worldPoint };
      setMeasureLine({ start: worldPoint, end: worldPoint });
      onSelectToken(null);
      return;
    }
    if (scenePreferences.toolMode === "fog") {
      const cell = screenToCell(point);
      const key = `${cell.x},${cell.y}`;
      const painting = !fogMapRef.current.has(key);
      dragRef.current = { type: "fog", painting };
      toggleFogCell(key, painting);
      onSelectToken(null);
      return;
    }
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (drag.type === "measure") {
      const worldPoint = screenToWorld(point);
      dragRef.current = { ...drag, endWorld: worldPoint };
      setMeasureLine({ start: drag.startWorld, end: worldPoint });
      return;
    }
    if (drag.type === "fog") {
      const cell = screenToCell(point);
      const key = `${cell.x},${cell.y}`;
      toggleFogCell(key, drag.painting);
      return;
    }
    if (drag.type === "token") {
  ...
  const handlePointerUp = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (drag?.type === "measure") {
      // keep line visible briefly then clear
      setTimeout(() => setMeasureLine(null), 2400);
    }
  ...
  useEffect(() => {
    ...
    if (fogLayer) {
      ...
      if (fogMap.size > 0) {
        ...
        fogMap.forEach((key) => {
          ...
          fogLayer.rect(gx * gs, gy * gs, gs, gs).fill({ color: 0x000000, alpha: 0.82 });
        });
      }
    }
    ...
    tokens.forEach((token) => {
      ...
      ring.circle(0, 0, tokenPx * 0.45).fill({ color: isSelected ? 0x4ad9d9 : 0x050a10, alpha: imageUrl ? (isSelected ? 0.2 : 0.08) : isSelected ? 0.42 : 0.78 });
      ring.circle(0, 0, tokenPx * 0.41).stroke({ color: token.visible_to_players === false ? 0xf59e0b : 0x4ad9d9, alpha: isSelected ? 1 : 0.55, width: 3 });
      ...
      // ── condition badge dots ──────────────────────────────────────────
      const conditions = tokenConditions(token)
        .filter((c) => !c.startsWith("visual:") && !c.startsWith("cutout:"))
        .slice(0, 5);
      if (conditions.length > 0) {
        const dotR = Math.max(5, tokenPx * 0.1);
        const totalW = conditions.length * dotR * 2 + (conditions.length - 1) * 3;
        conditions.forEach((cond, i) => {
          const color = CONDITION_HEX[cond.toLowerCase()] ?? 0x6b7280;
          const dot = new PIXI.Graphics();
          const cx = -totalW / 2 + i * (dotR * 2 + 3) + dotR;
          dot.circle(cx, -tokenPx * 0.5 - dotR - 2, dotR).fill({ color, alpha: 0.95 });
          dot.circle(cx, -tokenPx * 0.5 - dotR - 2, dotR).stroke({ color: 0xffffff, alpha: 0.25, width: 1.5 });
  ...
  }, [camera, fichasMap, fogMap, scenePreferences.gridOpacity, scenePreferences.gridSize, scenePreferences.showGrid, scenePreferences.toolMode, selectedTokenId, size.height, size.width, tokens]);

  // ── measure line (separate Pixi layer) ─────────────────────────────────
  useEffect(() => {
    const PIXI = pixiRef.current;
    const measureLayer = measureLayerRef.current;
    if (!PIXI || !measureLayer) return;
    measureLayer.clear();
    ...
    for (let i = 0; i < segments; i += 2) {
      ...
      measureLayer.moveTo(start.x + dx * t0, start.y + dy * t0);
      measureLayer.lineTo(start.x + dx * t1, start.y + dy * t1);
    }
    measureLayer.stroke({ width: 2.5, color: 0xfbbf24, alpha: 0.92 });
    // start dot
    measureLayer.circle(start.x, start.y, 5).fill({ color: 0xfbbf24, alpha: 1 });
    // end dot
    measureLayer.circle(end.x, end.y, 5).fill({ color: 0xfbbf24, alpha: 1 });
    // distance label background + text handled via DOM overlay (see JSX)
    void meters; // used in JSX
  }, [measureLine, scenePreferences.gridSize]);

  // compute measure distance for DOM overlay
  const measureDistanceLabel = (() => {
    if (!measureLine) return null;
    const gs = scenePreferences.gridSize || 50;
    const px = Math.hypot(measureLine.end.x - measureLine.start.x, measureLine.end.y - measureLine.start.y);
    const cells = px / gs;
    const meters = cells * METERS_PER_CELL;
    return { meters: meters.toFixed(1), cells: cells.toFixed(1) };
  })();

  const statusText = {
    map: "Reposicionando cena",
    pan: "Câmera livre",
    measure: "Régua de medição",
    fog: "Névoa de guerra",
    select: "Tokens cinemáticos",
  }[scenePreferences.toolMode] ?? "Tokens cinemáticos";

  return (
    <div
      ref={hostRef}
      className="fixed inset-0 overflow-hidden touch-none select-none bg-[#050a10]"
      style={{ cursor: scenePreferences.toolMode === "fog" ? "crosshair" : scenePreferences.toolMode === "measure" ? "crosshair" : "default" }}
    ...
    >
      ...
      {/* measure distance DOM label */}
      {measureLine && measureDistanceLabel && (
        <div
          className="pointer-events-none fixed z-50 rounded-2xl border border-yellow-400/40 bg-[#050a10]/90 px-3 py-1.5 text-center backdrop-blur-sm"
          style={{
            left: (measureLine.end.x - camera.x) * camera.zoom,
            top: (measureLine.end.y - camera.y) * camera.zoom - 44,
            transform: "translateX(-50%)",
          }}
        >
          <div className="text-sm font-black text-yellow-300">{measureDistanceLabel.meters}m</div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-yellow-500/70">{measureDistanceLabel.cells} quadrados</div>
        </div>
      )}

      {/* fog mode overlay hint */}
      {scenePreferences.toolMode === "fog" && (
        <div className="pointer-events-none fixed bottom-24 left-1/2 z-50 -translate-x-1/2 rounded-2xl border border-white/10 bg-[#050a10]/80 px-4 py-2 text-center backdrop-blur-sm">
          <div className="text-[10px] font-black uppercase tracking-widest text-white/60">
            Clique/arraste para cobrir · Clique em célula coberta para revelar
          </div>
        </div>
      )}
      ...
      <div className="fixed left-1/2 top-[92px] z-40 flex w-[min(calc(100vw-1rem),560px)] -translate-x-1/2 items-center justify-between gap-2 rounded-[26px] border border-[#4ad9d9]/20 bg-[#050a10]/88 px-3 py-2 shadow-2xl backdrop-blur-xl md:top-4">
        ...
        <div className="text-right">
          ...
          <div className="mt-1 text-[9px] uppercase tracking-[0.16em] text-slate-400">{statusText}</div>
        </div>
      </div>
